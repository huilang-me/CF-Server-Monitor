...
const translations = reactive({
  en: {
    ...
    invalidAgentBaseUrl: 'Invalid Agent BaseURL. Use an HTTP(S) URL without credentials, query parameters, or fragments.',
    invalidAgentProxyUrl: 'Invalid proxy URL. Use an HTTP(S) proxy origin with optional URL-encoded credentials.',
    ...
    agentBaseUrlPlaceholder: 'Blank uses the current API endpoint',
    agentBaseUrlHint: 'Root URL for agent scripts, /update reports, and automatic updates. Optional path prefixes are supported.',
    agentProxyUrl: 'HTTP(S) Proxy',
    agentProxyUrlPlaceholder: 'http://user:password@proxy.example.com:8080',
    agentProxyUrlHint: 'Used only for requests from this agent to its BaseURL. Ping, public IP, and region checks stay direct.',
    agentNetworkInstallWarning: 'This setting is only written by the generated install command. Reinstall the agent after changing it.',
    ...
    exportProxyCredentialsWarning: 'The export includes full agent proxy URLs and plaintext proxy credentials.',
    exportProxyCredentialsConfirm: 'The exported JSON will contain plaintext proxy credentials. Continue?',
  ...
  zh: {
    ...
    invalidAgentBaseUrl: '探针 BaseURL 无效，请使用不含账号密码、查询参数和片段的 HTTP(S) 地址。',
    invalidAgentProxyUrl: '代理地址无效，请使用 HTTP(S) 代理根地址；账号密码需使用 URL 编码。',
    ...
    agentNetwork: '探针网络',
    installOnly: '仅安装生效',
    agentBaseUrl: '探针 BaseURL',
    agentBaseUrlPlaceholder: '留空使用当前 API 地址',
    agentBaseUrlHint: '统一用于探针脚本、/update 上报与自动更新，支持路径前缀。',
    agentProxyUrl: 'HTTP(S) 代理',
    agentProxyUrlPlaceholder: 'http://用户名:密码@proxy.example.com:8080',
    agentProxyUrlHint: '仅代理该探针到 BaseURL 的请求，Ping、公网 IP 与地区检测保持直连。',
    agentNetworkInstallWarning: '此配置只会写入新生成的安装命令；修改后必须重新安装探针才能生效。',
    rxCorrection: '下行流量校正',
    txCorrection: '上行流量校正',
    ...
    exportProxyCredentialsWarning: '导出文件会包含完整探针代理地址和明文代理账号密码。',
    exportProxyCredentialsConfirm: '导出的 JSON 将包含明文代理账号密码，确定继续吗？',

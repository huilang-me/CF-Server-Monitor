...
const translations = reactive({
  en: {
    ...
    wssReportTip: 'Enabling WSS reduces Workers requests and lowers latency by about 15x, but it can use 80%+ of Durable Objects duration regardless of server count. You may receive an overage email around midnight every day. Recommended to enable.',
    ...
    notifyTypeAuto: 'Auto-detect (by URL)',
    notifyTypeCustomHttp: 'Custom HTTP Webhook',
    customHttpUrl: 'Webhook URL',
    customHttpAuth: 'Authorization Value',
    customHttpTip: 'Sends POST with JSON body {"title":...,"content":...} plus an "Authorization" header. Method is fixed to POST.',
    customHttpUrlRequired: 'Webhook URL is required when notifications are enabled',
    ...
    wssReportDisabledConnectionHint: 'Agent WSS reporting is disabled globally, so this server is forced to HTTP POST.',
  ...
  zh: {
    ...
    wssReportTip: '开启 WSS 可减少 Workers 请求量，降低约 15 倍延迟；会占用 80%+ Durable Objects 时长消耗（和服务器数量无关），每天半夜可能收到一封超额邮件，建议开启。',
    publicAccess: '公开访问',
    showPrice: '显示价格',
    showExpire: '显示到期时间',
    showTf: '显示流量配额',
    ...
    chatId: '聊天 ID',
    notifyType: '通知类型',
    notifyTypeAuto: '自动识别（按 URL）',
    notifyTypeCustomHttp: '自定义 HTTP Webhook',
    customHttpUrl: 'Webhook 地址',
    customHttpAuth: 'Authorization 值',
    customHttpTip: '以 POST 发送 JSON 体 {"title":...,"content":...} 并附带 "Authorization" 请求头，方法固定为 POST。',
    customHttpUrlRequired: '启用通知时必须填写 Webhook 地址',
    saveConfig: '保存配置',
    saving: '保存中...',
    hostnameLabel: '主机名',
    groupName: '分组名称',
    ...
    wssReportDisabledConnectionHint: 'Agent WSS 上报已全局关闭，此服务器会强制使用 HTTP POST。',
    targetOs: '目标系统',
    copied: '已复制！',
    close: '关闭',
